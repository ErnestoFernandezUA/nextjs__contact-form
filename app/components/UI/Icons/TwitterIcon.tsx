...
export const TwitterIcon: React.FC<TwitterIconProps>  = ({ theme = Theme.dark }) => {
  ...
  const baseColors = theme === Theme.dark 
    ? { circle: '#1B1B1B', path: 'white' } 
    : { circle: 'white', path: '#1B1B1B' };

  const activeColors = {
    circle: baseColors.path,
    path: baseColors.circle
  };

  const circleFill = isActive ? activeColors.circle : baseColors.circle;
  const pathFill = isActive ? activeColors.path : baseColors.path;
